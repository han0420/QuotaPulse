...
#if !compiler(>=6.2)
            // macOS 15's material is less color-neutral than Liquid Glass.
            // A cool frost layer keeps weather colors atmospheric without
            // letting a cloudy palette dominate the panel.
            LinearGradient(
                colors: [
                    Color.white.opacity(0.88),
                    Color(red: 0.97, green: 0.985, blue: 1.00).opacity(0.78)
                ],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
...
private struct WeatherMotionCanvas: View {
    ...
    private func drawClear(in context: GraphicsContext, size: CGSize, phase: TimeInterval) {
        ...
        glow.fill(
            ...
            with: .color((isDay ? Color(red: 0.78, green: 0.86, blue: 1.00) : Color.white).opacity(0.16))
        )
        context.fill(
