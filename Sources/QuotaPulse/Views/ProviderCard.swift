...
struct ProviderCard: View {
    ...
    private var activityLabel: some View {
        ...
        .foregroundStyle(isConsuming ? provider.accent : AppColors.secondaryText)
    }
    ...
    private var activityIndicator: some View {
        TimelineView(.animation(minimumInterval: isConsuming && !reduceMotion ? 1 / 20 : 1)) { timeline in
            ...
            ZStack {
                Circle()
                    .fill(isConsuming ? provider.accent.opacity(0.16) : AppColors.stable.opacity(0.16))
                Circle()
                    .fill(isConsuming ? provider.accent : AppColors.stable)
